...
var ContentDatabaseHandler = class {
  ...
  getCostSummary(specificPath) {
    ...
    for (const [path6, entry] of Object.entries(entries)) {
      if (!entry?.costAccounting) continue;
      const costs = entry.costAccounting;
      costsByContent[path6] = costs;
...
var Shakespeare = class _Shakespeare {
  ...
  getContentNeedingReview() {
    ...
    const database = this._db.getData();
    return Object.entries(database.entries || {}).filter(([_, entry]) => entry.status === "needs_review").map(([path6, _]) => path6);
  }
  /**
   ...
   */
  async reviewContent(path6) {
    const result = await this.reviewContentBatch([path6], 1);
  ...
  getWorstScoringContent() {
    ...
    let worstPath = null;
    for (const [path6, entry] of Object.entries(database.entries)) {
  ...
  async reviewAllBatch(batchSize = 5) {
    ...
    const contentNeedingReview = allEntries.filter(([, entry]) => entry.status === "needs_review").map(([path6]) => path6);
  ...
  async improveWorstBatch(count = 5, batchSize = 3) {
    ...
    const scoredEntries = entries.filter(([, entry]) => entry.status !== "needs_review" && entry.status !== "meets_targets").map(([path6, entry]) => {
      const scores = Object.values(entry.currentScores);
      const avgScore = scores.reduce((a, b) => a + b, 0) / scores.length;
      return { path: path6, avgScore, entry };
...
async function main() {
  const contentDir = process.env.CONTENT_DIR || path5.join(process.cwd(), "content");
  const dbPath = process.env.DB_PATH || path5.join(process.cwd(), ".shakespeare", "content-db.json");
